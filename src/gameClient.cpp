    ...
    while (running)
    {
        ...
        if (!chosenPiece.isEmpty())
        {
            ...
            for (int x{}; x < 8; x++)
            {
                for (int y{}; y < 8; y++)
                {
                    if (isMoveValid(chosenX, chosenY, x, y, localBoard))
                    {
...
bool GameClient::isPieceBlockingTarget(int startX, int startY, int targetX, int targetY, std::array<std::array<Piece, 8>, 8> localBoard)
{
...
bool GameClient::isMoveValid(int startX, int startY, int targetX, int targetY, std::array<std::array<Piece, 8>, 8> localBoard)
{
